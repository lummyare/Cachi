            ...
            div {
                div {
                    ...
                    case .all:
                        ...
                        let e1 = div { "\(testsCount) tests" }.inlineBlock()
                        let e2 = HTMLBuilder.buildBlock(
                            div { " with \(testsFailedCount) failures" }.inlineBlock(),
                            div { " (\(testsFailedCount.percentageString(total: testsCount, decimalDigits: 1)))" }.inlineBlock()
                        )
                        let e3 = div { "in \(hoursMinutesSeconds(in: result.totalExecutionTime))" }.inlineBlock()
                        
                        return testsFailedCount > 0 ? HTMLBuilder.buildBlock(e1, e2, e3) : HTMLBuilder.buildBlock(e1, e3)
                    case .passed:
                ...
                forEach(ShowFilter.allCases) { filterCase in
                    link(url: "\(self.path)?id=\(result.identifier)\(filterCase.params())") { filterCase.rawValue.capitalized }.class(showFilter == filterCase ? "button-selected" : "button")
                }
